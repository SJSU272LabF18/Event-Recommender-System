...
/*                 this.state.propertyData.map(property => { 
 ...
 /*                console.log("prop data in cdm" + this.props.propertyData);
    ...
    handleBookNow = async (e) => {
        console.log("handlebookingnow oemailid" + this.props.propertyData[0].oemailid);
        var values = {
        ...
        await axios.post('http://localhost:3001/addbooking',values)
            .then(response => {
                if(response.status == 200){
                    this.setState({
                        booked : true
...
sendMessageHandler = async (e) => {
    ...
    swal("Message sent to the owner!", "Please check your inbox in Profile","success");
}
...
{/*                     <label for="exampleFormControlTextarea6">Shadow and placeholder</label> */}
                    <textarea onChange = {this.inputHandler} class="form-control-book-now z-depth-1" id="exampleFormControlTextarea6" rows="3" placeholder="Write something here..."></textarea>
                    </div>
...
export default connect(mapStateToProps,{travelerpropertydetails, travelerSendMessage})(TravelerPropertyDetails);