...
class Recommendations extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    console.log(
      "Fetch flag in search results " + nextProps.fetchproperty.length
    );
  }
  ...
  async componentDidMount() {
    ...
    await axios
      .post("http://52.9.107.69:3001/getevents", values)
      .then(response => {
    ...
    if (this.state.propertiesdata.length === 0) {
      ...
      for (let i = 0; i < this.state.propertiesdata.length; i++) {
        axios
          .post(
            "http://52.9.107.69:3001/download/" +
              this.state.propertiesdata[i].picturelist.split(",")[0]
          )
          .then(response => {
  ...
  render() {
    ...
    const paginatedproperties = this.state.propertiesdata.slice(
      indexOfFirstPage,
      indexOfLastPage
    );
    ...
    for (
      let i = 1;
      i <= Math.ceil(this.state.propertiesdata.length / itemsPerPage);
      i++
    ) {
      pageNumbers.push(i);
    }
              ...
              <div class="container list-props">
                <div class="row jumbotron-traveler">
                  <div class="column left">
                    <div>
                      <img class="imagesearch" src={data.picturelist} />
                    </div>
                  </div>
                    ...
                    <div>
                      <td>
                        <h3>
                          <Link
                            to="/viewevent"
                            onClick={this.handleViewPropertyDetails.bind(
                              this,
                              data._id
                            )}
                          >
...
function mapDispatchToProps(dispatch) {
  return bindActionCreators(
    { searchProperties, fetchpropertydetails },
    dispatch
  );
}
