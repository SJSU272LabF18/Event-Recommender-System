...
class Profile extends Component {
  ...
  componentDidMount() {
    ...
    axios.get("http://52.9.107.69:3001/getprofile/").then((response, err) => {
      ...
      axios
        .post("http://52.9.107.69:3001/download/" + this.state.profilepic)
        .then(response => {
  ...
  onChange = e => {
    ...
    axios.post("http://52.9.107.69:3001/photos", formData).then(result => {
      if (result.status === 200) {
        console.log("res data!!!" + result.data);
        axios
          .post("http://52.9.107.69:3001/download/" + result.data)
          .then(response => {
        ...
        <form>
          <div className="profile-form-inner">
            ...
            <div>
              <input
                type="text"
                onChange={this.handleCityCountry}
                class="form-control input-lg js-input-field"
                id="profileCityCountryInput"
                placeholder="My City, My State"
    ...
    return (
      <div>
        ...
        <div class="text-center">
          ...
          <p class="text-muted">
            <span class="user-location" />Member since 2018
          </p>
        </div>

        <div className="profile-form-main">{profileDetails}</div>
      </div>
    );
  }
}
