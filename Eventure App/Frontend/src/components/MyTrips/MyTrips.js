...
class MyTrips extends Component {
    ...
    async componentDidMount(){
           ...
           /*  axios.get('http://localhost:3001/getmytrips/')
                .then(response => { */
                this.setState({
    ...
    handleSearch = (e) => {
        if(e.target.value)
        {
        let filteredresult = _.filter(this.state.tripsdata, trip => trip.headline.toLowerCase().includes(e.target.value.toLowerCase()))
...
handleSearchStartDate = (e) => {
    if(e.target.value)
    {
    let filteredresult = _.filter(this.state.tripsdata, trip => trip.bookedstartdate == e.target.value)
    console.log("Filtered result " + JSON.stringify(filteredresult))
    this.setState({
        tripsdata: filteredresult
    })
...
handleSearchEndDate = (e) => {
    if(e.target.value)
    {
    let filteredresult = _.filter(this.state.tripsdata, trip => trip.bookedenddate ==e.target.value)
    console.log("Filtered result " + JSON.stringify(filteredresult))
    this.setState({
        tripsdata: filteredresult
    })
...
clickHandler(event) {
    ...
    render()
    {
        ...
        const paginatedtrips = this.state.tripsdata.slice(indexOfFirstPage, indexOfLastPage);
        console.log("Number of properties : " + this.state.tripsdata.length);
        
        const pageNumbers = [];
        for (let i = 1; i <= Math.ceil(this.state.tripsdata.length / itemsPerPage); i++) {
          pageNumbers.push(i);
        }
        ...
        let details = paginatedtrips.map((data,i) => {
            return(
                ...
                <tr key={i}>
                    ...
                    <div><td>Trip start : {data.bookedstartdate}</td></div>
                    <div><td>Trip end :{data.bookedenddate}</td></div>
...
export default connect(mapStateToProps, {getmytrips})(MyTrips);