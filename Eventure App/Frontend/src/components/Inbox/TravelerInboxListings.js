...
class TravelerInboxListings extends Component {
    constructor(props) {
        super(props);
        this.state = {
            messagelistdata : []
        ...
        console.log("this" + this.state.messagelistdata)
         let details = this.state.messagelistdata.map((message, i) => {
            return (
                ...
                <span> <Link to={`/travelerinbox/${message._id}`} onClick = {this.handleViewMessageDetails.bind(this,message._id)}>{message.headline}</Link></span><br></br>
                <hr></hr>  
                <h5> Owner email: {message.oemailid}</h5>  
                <span>Property at: {message.address}</span><br></br>  
...
export default connect(mapStateToProps, { fetchmessagelist,fetchmessagethreaddetails })(TravelerInboxListings);