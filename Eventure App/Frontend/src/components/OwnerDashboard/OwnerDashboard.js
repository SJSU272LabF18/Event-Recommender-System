...
class OwnerDashboard extends Component {
  ...
  handleSearch = e => {
    if (e.target.value) {
      let newlyDisplayed = _.filter(this.state.dashboarddata, booking =>
        booking.headline.toLowerCase().includes(e.target.value.toLowerCase())
      );
  ...
  handleSearchStartDate = e => {
    if (e.target.value) {
      let filteredresult = _.filter(
        ...
        booking => booking.bookedstartdate == e.target.value
      );
  ...
  handleSearchEndDate = e => {
    if (e.target.value) {
      let filteredresult = _.filter(
        this.state.dashboarddata,
        booking => booking.bookedenddate == e.target.value
      );
      console.log("Filtered result " + JSON.stringify(filteredresult));
      this.setState({
        dashboarddata: filteredresult
  ...
  async componentDidMount() {
    ...
    /*  axios.get('http://52.9.107.69:3001/getownerdashboard/' + data.oemailid)
                .then(response => { */
  ...
  render() {
    ...
    const paginatedbookings = this.state.dashboarddata.slice(
      firstpageindex,
      lastpageindex
    );
    ...
    for (
      let i = 1;
      i <= Math.ceil(this.state.dashboarddata.length / itemsPerPage);
      i++
    ) {
      pageNumbers.push(i);
    }
          ...
          <tbody class="tableprops">
            <tr key={i}>
              <div class="container list-props">
                ...
                <div>
                  <td>Booked from : {data.bookedstartdate}</td>
                </div>
                <div>
                  <td>Booked till :{data.bookedenddate}</td>
                </div>
                <div>
                  <td>Price : {data.price}</td>
                </div>
                <div>
                  <td>Booked by : {data.temailid}</td>
                </div>
              </div>
              <br />
            </tr>
    ...
    return (
      <div>
        ...
        <div class="container">
          ...
          <br />
          <div className="flex-container-ownerdasboard">{details}</div>
