...
app.use(cors({ origin: "http://52.9.107.69:3000", credentials: true }));
...
app.use(function(req, res, next) {
  res.setHeader("Access-Control-Allow-Origin", "http://52.9.107.69:3000");
...
app.post("/searchevents", async function(req, res) {
  console.log(
    "Inside search" + req.body.eventstartdate + req.body.eventcitystate
  );
...
app.post("/getevents", async function(req, res) {
  ...
  Events.find({ _id: { $in: includeEventsArray } }, function(err, events) {
    if (err) {
      console.log(err);
      res.send(err);
    } else {
